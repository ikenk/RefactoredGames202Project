...
export function loadGLTF(
  ...
  new GLTFLoader(manager).setPath(path).load(name + '.gltf', function (gltf: GLTF) {
    // console.log(gltf)
    gltf.scene.traverse(async function (child) {
      ...
      if (child.type === 'Mesh') {
        ...
        if (material.map != null) {
          ...
          diffuseMap.CreateConstantTexture(renderer.gl, material.color.toArray(), true)
        }
        ...
        let ssrMaterial, shadowMaterial, bufferMaterial
